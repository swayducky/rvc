...
model_files = [
    f
    for f in files
    if f.startswith(MODELNAME) and has_numbers(f) and "G_" not in f and "D_" not in f
]
...
if GDLoading:
    ...
    shutil.move(f"{source_dir}/D_{GNumber}.pth", f"logs/{MODELNAME}/G_{MODELSTEP}.pth")

    shutil.move(
        f"{source_dir}/G_{MODELSTEP}.pth", f"logs/{MODELNAME}/D_{MODELSTEP}.pth"
    )
